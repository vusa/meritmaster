...
import za.co.merits.event.StudentCoursePayload;
import za.co.merits.model.*;
import za.co.merits.model.Person;
import za.co.merits.qualifier.DataRepository;
import za.co.merits.qualifier.StudentAdded;
import za.co.merits.qualifier.StudentRemoved;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Implements the {@link MeritsFacade} as a {@link Stateless} EJB to take advantage
 ...
 */
@Stateless
public class MeritsFacade{

	private static Logger log = LoggerFactory.getLogger(MeritsFacade.class);
	...
	/* (non-Javadoc)
	 * @see za.co.merits.bean.MeritsFacade#searchStudents(za.co.merits.bean.SearchCriteria, int, int)
	 */
	//@SuppressWarnings("unchecked")
	///@Override
	public List<Student> searchStudents(SearchCriteria criteria,
		...
		String sql = "select distinct s from Student s left JOIN s.courses c left join c.teacher t";
    ...
    public List<Assignment> getAssignmentsForCourse(Course c){
        String ql = "select a from Assignment a where a.course.id = :courseId order by a.issueDate desc";
        return  entityManager.createQuery(ql).setParameter("courseId", c.getId()).getResultList();
    }

    public List<AssignmentScore> getAssignmentScoresForStudent(Student s){
        String ql = "select a from AssignmentScore a where a.student.id = :studentId order by a.submitDate desc";
        return  entityManager.createQuery(ql).setParameter("studentId",s.getId()).getResultList();
    }
	...
	public List<TeacherCourseCount> getTeachCourseCounts() {
		List<TeacherCourseCount> results = new ArrayList<>();
