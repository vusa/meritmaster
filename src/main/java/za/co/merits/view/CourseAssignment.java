 ...
 * <p/>
 * We inject the {@link TeacherHome} bean to access the current person. Bear in
...
public class CourseAssignment implements Serializable {
	...
	public List<Course> getAvailableCourses() {
		if (availableCourses == null) {
			availableCourses = meritsFacade.getCourses();
			List<Course> enrolled = getStudent().getCourses();
	...
	public void addStudentToSelectedCourses() {
		for (String id : selectedAvailableCourses) {
			Course course = meritsFacade.getCourse(Long.valueOf(id));
			getStudent().getCourses().add(course);
	...
	public void removeStudentFromSelectedCourses() {
		for (String id : selectedEnrolledCourses) {
			Course course = meritsFacade.getCourse(Long.valueOf(id));
			getStudent().getCourses().remove(course);
